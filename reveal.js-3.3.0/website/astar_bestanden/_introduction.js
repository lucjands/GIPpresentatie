...
var DiagramLayer = (function () {
    ...
    DiagramLayer.prototype.currently_pointing_at = function () {
        ...
        // handled at all here.
        var pointer = d3.mouse(this.svg.node());
        var id = this.diagram.layout.pixel_to_tile(pointer);
        ...
        // the translate.
        // Fallback #1 is used only if pixel_to_tile returns -1 (e.g.
        ...
        // Fallback #2 is used when the browser supports
        // SVG.getIntersectionList (webkit/ie/opera but not firefox)
        if (id < 0) {
            var svg_root = this.diagram.svg_root.node();
            if (svg_root.getIntersectionList) {
                var query = svg_root.createSVGRect();
                query.x = pointer[0];
                query.y = pointer[1];
                query.width = 1;
                query.height = 1;
                var result = svg_root.getIntersectionList(query, this.diagram.layer.base.svg.node());
                if (result.length > 0) {
                    // Pick the first match; there shouldn't be any
                    // overlaps in the maps I've made
                    id = d3.select(result[0]).data()[0];
                }
            }
            else {
            }
        }
        // TODO: Fallback #3: try document.getElementFromPoint
        // (firefox/webkit, not sure about use in svg for opera/ie)
        return id;
    };
    return DiagramLayer;
}());
...
var GraphEditorLayer = (function (_super) {
    ...
    function GraphEditorLayer(diagram, cycle_order) {
        ...
        this.diagram.layer.base.svg.classed('editable', true);
...
var NodeLayer = (function (_super) {
    __extends(NodeLayer, _super);
    // maxWeight is exclusive
    function NodeLayer(diagram, radius, maxWeight) {
        var _this = this;
        if (maxWeight === void 0) { maxWeight = Infinity; }
        _super.call(this, diagram, 'nodes');
        this.diagram = diagram;
        this.radius = radius;
        this.maxWeight = maxWeight;
        this.svg.selectAll(".node").data(this.diagram.graph.tiles_less_than_weight(maxWeight))
            .enter().append('circle')
            .attr('class', "node")
            .attr('transform', function (id) { return "translate(" + _this.diagram.layout.tile_center(id) + ")"; })
            .attr('r', radius);
...
var EdgeLayer = (function (_super) {
    ...
    function EdgeLayer(diagram, maxWeight) {
        ...
        this.diagram = diagram;
        this.svg.selectAll(".edge").data(this.diagram.graph.all_edges(maxWeight))
            .enter().append('path')
            .attr('class', "edge")
...
var IntroBackgroundLayer = (function (_super) {
    __extends(IntroBackgroundLayer, _super);
    function IntroBackgroundLayer(diagram) {
        _super.call(this, diagram, 'intro-map-background');
        this.diagram = diagram;
...
makeHoverButton("#show-graphs1-different-map", "#diagram-graphs1-under", "alternate");
